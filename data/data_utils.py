...
def visualize_biparite(G, u, v, fig_path = "temp.png"):
    
    import matplotlib
    import matplotlib.pyplot as plt
    # nx.draw(G)
    G_left = G.subgraph([i for i in range(u)])
    G_right = G.subgraph([i for i in range(u,u+v)])
    edges = G.edges()
    
    pos = dict()
    pos.update( (n, (1, i)) for i, n in enumerate(G_left) ) # put nodes from X at x=1
    pos.update( (n, (2, i)) for i, n in enumerate(G_right) ) # put nodes from Y at x=2
